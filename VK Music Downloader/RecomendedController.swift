//
//  RecomendedController.swift
//  VK Music Downloader
//
//  Created by iOS on 12/10/15.
...
class RecomendedController: UIViewController, UITableViewDataSource, UITableViewDelegate,UISearchBarDelegate, IGLDropDownMenuDelegate, DownloadManagerDelegate {
    @IBOutlet weak var myTableView: UITableView!
    var currentAudioIndex:Int!
    var searchBar = UISearchBar()
    var currentGroupId : NSInteger!
    var trackId : NSInteger!
    ...
    @IBAction func searchButtonClicked(sender: AnyObject) {
        ...
        if (searchBarButtonItem.title=="Menu"){
            ...
            NSUserDefaults.standardUserDefaults().setObject(nil, forKey: "favskey2")
    ...
    override func viewDidLoad() {
        ...
        NSUserDefaults.standardUserDefaults().setObject(nil, forKey: "favskey2")
        NSUserDefaults.standardUserDefaults().setObject(nil, forKey: "download2")
        ...
        self.updateTableCellOnChangeCell(notification)
            ...
            loadMusicForTrack(currentGroupId)
    ...
    func loadMoreMusicForTrackWithOffset(offset:NSInteger){
        whatToShow=0
        print("offset \(self.trackOffset)")
        ...
        let audioReq: VKRequest = VKRequest(method: "audio.getRecommendations", andParameters: ["target_audio" : String(self.currentGroupId)+"_"+String(self.trackId),"count":"20","offset":String(offset)], andHttpMethod: "POST")
        audioReq.executeWithResultBlock(
            {
                ...
                if(mylocalArray.count>0){
                    ...
                    print("loadMoreMusicForGroupWithOffset: \(self.trackOffset)")
                    ...
                    print("local: no audio data for this group: \(self.trackOffset)")
                    self.loadMore = false
    ...
    func loadMusicForTrack(dict:NSInteger){
        whatToShow=0
        ...
        let audioReq: VKRequest = VKRequest(method: "audio.getRecommendations", andParameters: ["target_audio" : String(dict)+"_"+String(self.trackId),"count":"20","offset":"0"], andHttpMethod: "POST")
        ...
        audioReq.executeWithResultBlock(
            {
                ...
                if(mylocalArray.count>0){
                    ...
                    print("loadMusicForGroup: \(self.trackOffset)")
                    ...
                    self.errorTextEmptyTable = String("No recommended audio files.\nOr something went wrong.")
    ...
    func tableView(tableView: UITableView, willDisplayCell cell: UITableViewCell, forRowAtIndexPath indexPath: NSIndexPath) {
        if (indexPath.row == self.allDataSource.count - 1)
        {
                ...
                if(loadMore){
                    self.loadMoreMusicForTrackWithOffset(self.trackOffset)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if(editMode){
            ...
            if cell!.accessoryType == UITableViewCellAccessoryType.None {
                ...
                if let favs : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("favskey2") as? NSMutableArray{
                    if(favs.count==0){
    ...
    func updateFavoriteInUserDefaultsFor(index:NSDictionary,value:Bool){
        if  let oldFavs : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("favskey2") as? NSMutableArray{
            ...
            NSUserDefaults.standardUserDefaults().setObject(newFavorites, forKey: "favskey2")
            NSUserDefaults.standardUserDefaults().synchronize()
        }else{
    ...
    func updateDownloadInUserDefaultsFor(index:NSDictionary,value:Bool){
        if  let oldFavs : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("download2") as? NSMutableArray{
            ...
            NSUserDefaults.standardUserDefaults().setObject(newFavorites, forKey: "download2")
            NSUserDefaults.standardUserDefaults().synchronize()
        }else{
            let newFavorites : NSMutableArray = NSMutableArray()
    ...
    func isDown(index:NSDictionary)->Bool{
        if let favs : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("download2") as? NSMutableArray{
            if (favs.count>0){
        ...
        if let favs : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("favskey2") as? NSMutableArray{
            if (favs.count>0){
                ...
                for var i = 0 ; i < favs.count ; i++ {
                    ...
                    if let favs1 : NSMutableArray = NSUserDefaults.standardUserDefaults().objectForKey("favskey2") as? NSMutableArray{
                    if(favs1.count==0){
    ...
    func downloadManager(downloadManager: DownloadManager!, downloadDidFail download: Download!) {
        print("downloadDidFail \(download.error.localizedDescription)")
